...
def make_multilinear_gradient_group(live_graphs, group_indicator, target_nodes, selectable_nodes, p_attend):
    ...
    def gradient(x, batch_size):
        ...
        temp=gradient_estimate_all_nodes(x, Gs, Ps, start_poses, ws, batch_size)
        if temp is not None and is_matrix_all_zero(temp)==False and is_matrix_all_zero(group_indicator)==False:
            grad = temp @ group_indicator
            return grad[selectable_nodes, :]
        else:
            gradient(x,batch_size)
    return gradient
...
def gradient_coverage_single_edgelist(x, G, P, start_pos, w, v):
    ...
    for j in range(start_pos[v], start_pos[v+1]):
        ...
        if p_all_fail == 0:
            ...
            #判断divisionbyzero error
            if(1 - x[G[j]]*P[j])!=0:
                p_others_fail = p_all_fail/(1 - x[G[j]]*P[j])
            else:
                p_others_fail = p_all_fail/0.1
...
def gradient_estimate_all_nodes(x, Gs, Ps, start_poses, ws, B):
    ...
    for b in range(B):
        ...
        """
        temp = gradient_coverage_single_edgelist(x, Gs[idx], Ps[idx], start_poses[idx], ws[idx], v)
        print(temp)
        if temp:
            print("(n/float(B)) * temp",(n/float(B)) * temp)
        """
        grad_est[:, v] += (n/float(B)) * gradient_coverage_single_edgelist(x, Gs[idx], Ps[idx], start_poses[idx], ws[idx], v)
    #print(grad_est)
    if is_matrix_all_zero(grad_est)==False:
        return grad_est
    else:
       gradient_estimate_all_nodes(x, Gs, Ps, start_poses, ws, B) 
