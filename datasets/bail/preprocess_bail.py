...
with open('bail_edges.txt', 'r') as f:
    edges = f.readlines()
...
with open('bail_relationship.G', 'wb') as f:
    pickle.dump(graph, f)
...
for row in rows:
    ...
    if key in list(mapping.keys()):
        ...
        #print(values_1[1][0])
        my_dict_att[mapping[key]]=values_1[0][0]
...
with open('bail_prob.dic', 'wb') as f:
...
for edge in edges:
    ...
    empty_arr=[]
    empty_arr.append(np.outer(list(reader.iloc[source])[6:10], list(reader.iloc[target])[6:10]))
